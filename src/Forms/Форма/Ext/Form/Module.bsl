...
Процедура КомандаОтправитьКодАвторизации()
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ОтправитьКодАвторизации(Параметр_code));
КонецПроцедуры
...
&НаСервере
Процедура ОтправитьСообщениеВМакс()
...
&НаСервере
Процедура ВыбратьКонтакт(ИдКонтакта)
	
	Ответ = ОбработкаОбъект().ОтправитьКонтакт(Параметр_chatId, ИдКонтакта);
	КонсольОтправка = СтруктураВJSONСтроку(Ответ);
	
	Сообщить("ru = 'Контакт отправлен успешно. idMessage='; en = 'Contact successfully sent. idMessage='" + Ответ.idMessage);
...
Процедура КомандаСоздатьГруппу()	
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().СоздатьГруппу(Параметр_groupName, Параметр_chatIds.ВыгрузитьЗначения()));
	
	Если ЗначениеЗаполнено(Консоль) Тогда	
		Параметр_chatId = СтруктураИзJSON(Консоль).chatId;
	КонецЕсли;	
...
Процедура КомандаИзменитьИмяГруппы()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ИзменитьИмяГруппы(Параметр_chatId, Параметр_groupName));
...
Процедура КомандаПолучитьИнформациюОГруппе()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ПолучитьИнформациюОГруппе(Параметр_chatId));
...
Процедура КомандаДобавитьУчастникаВГруппу()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ДобавитьУчастникаВГруппу(Параметр_chatId, Параметр_participantChatId));
...
Процедура КомандаУдалитьУчастникаИзГруппы()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().УдалитьУчастникаИзГруппы(Параметр_chatId, Параметр_participantChatId));
...
Процедура КомандаНазначитьПраваАдминистратораГруппы()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().НазначитьПраваАдминистратораГруппы(Параметр_chatId, Параметр_participantChatId));
...
Процедура КомандаОтозватьПраваАдминистратораГруппы()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ОтозватьПраваАдминистратораГруппы(Параметр_chatId, Параметр_participantChatId));
...
Процедура КомандаВыйтиИзГруппы()
	...
	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ВыйтиИзГруппы(Параметр_chatId));
...
&НаСервере
Процедура ПолучитьВсеИнстансыМакс()                                                                                       
...
&НаСервере
Процедура КомандаПроверитьНаличиеМакс()

	Консоль = СтруктураВJSONСтроку(ОбработкаОбъект().ПроверитьНаличиеМакс(Число(Параметр_phoneNumber)));
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ЗначениеЗаполнено(ВыбранноеЗначение) И ПроверитьЗаполнение() Тогда
		...
		ИначеЕсли ВыбранноеЗначение.ИмяФормы = "ВыборКонтакта" Тогда
			ВыбратьКонтакт(ВыбранноеЗначение.chatID)
...
Процедура ДекорацияКакПолучитьChatIdНажатие(Элемент)
	...
	ПерейтиПоНавигационнойСсылке("https://green-api.com/v3/docs/api/chat-id/");
...
Процедура ПереключательТипаЧатаПриИзменении(Элемент = Неопределено)
	...
	Элементы.ИдЧата.Заголовок = ЗаголовокЧата;
	Элементы.ИдЧатаСервисные.Заголовок = ЗаголовокЧата;
	Элементы.ИдЧатаЖурнала.Заголовок = ЗаголовокЧата;
	Элементы.ИдЧатаОтметка.Заголовок = ЗаголовокЧата;
...
Процедура apiUrlОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)	
	Объект.host = СтрЗаменить(СтрЗаменить(Текст, 
			"https://", ""),
			"/v3", "");
КонецПроцедуры
...
Процедура mediaUrlОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	Объект.mediaHost = СтрЗаменить(Текст, "https://", "");	
КонецПроцедуры
...
Процедура ИзменитьИмяГруппы(Команда)
	...
	Если Не ЗначениеЗаполнено(Параметр_chatId) Тогда	
		ПоказатьСообщениеПользователю("Поле ""ID группы"" не заполнено", "Параметр_chatId");
		Возврат;
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Параметр_groupName) Тогда	
		ПоказатьСообщениеПользователю("Поле ""Имя группы"" не заполнено", "Параметр_groupName");
...
Процедура ВыбратьФотоДляАватараГруппыСервер(АдресХранилища, ИмяФайла)
	...
	Ответ = ОбработкаОбъект().УстановитьФотоГруппы(Параметр_chatId, ИмяВременногоФайла);
...
&НаКлиенте
Процедура ПроверитьНаличиеМакс(Команда)

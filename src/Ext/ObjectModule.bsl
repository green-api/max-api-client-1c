...
// Запускает процесс получения сообщений.
// Результаты выполнения метода обрабатываются методами ниже
//
...
// https://green-api.com/v3/docs/api/receiving/notifications-format/incoming-message/ImageMessage/
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
//  Данные - Структура - Структура полей сообщения в формате JSON. См подробнее в документации
...
// https://green-api.com/v3/docs/api/receiving/notifications-format/outgoing-message/OutgoingMessageStatus/
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
//  Данные - Структура - Структура полей сообщения в формате JSON. См подробнее в документации
...
// https://green-api.com/v3/docs/api/receiving/notifications-format/StateInstanceChanged/
//
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
...
// https//green-api.com/v3/docs/api/receiving/notifications-format/DeviceInfo/
//
// Параметры,
//  Отказ - Булево -  Если Истина, то сообщение не будет подтверждено и вернется на сервер 
//    для повторного получения
...
// Стандартная функция-описание для подуключения к типовым конфигурациям
//
Функция СведенияОВнешнейОбработке() Экспорт   

	ВерсияВО = НомерВерсии();
	КраткоВО = "Green API-MAX " + ВерсияВО;
	ИнформацияВО     = "SDK GREEN API для работы с MAX из 1С";
	ИдентификаторВО = "GREENAPIMAX";
	
	// Чтобы не падала в конфигурациях где нет модулей 
...
//	Подробнее https://green-api.com/v3/docs/api/account/StartAuthorization/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция НачатьАвторизацию(НомерТелефона) Экспорт
	...
	Ответ = ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "startAuthorization"), Тело);
	Возврат Ответ;
...
//
// Параметры:
//	 КодИзСМС - строка - код который получил 
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьКодАвторизации(КодИзСМС) Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Структура = Новый Структура;
	Структура.Вставить("code", КодИзСМС);
	
	Тело = СериализоватьВJSONСтроку(Структура);
	
	Ответ = ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "sendAuthorizationCode"), Тело);
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для разлогинивания инстанса.
//	Подробнее https://green-api.com/v3/docs/api/account/Logout/ 
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция РазлогинитьАккаунт() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "logout"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для получения состояния инстанса.
//	Подробнее https://green-api.com/v3/docs/api/account/GetStateInstance/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьСостояниеАккаунта() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getStateInstance"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для перезапуска инстанса.
//	Подробнее https://green-api.com/v3/docs/api/account/Reboot/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПерезапуститьАккаунт() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "reboot"));
	Возврат Ответ;
      	
КонецФункции

// Метод предназначен для получения текущих настроек инстанса.
//	Подробнее https://green-api.com/v3/docs/api/account/GetSettings/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьНастройкиАккаунта() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getSettings"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для получения текущих настроек инстанса.
//	Подробнее https://green-api.com/v3/docs/api/account/setSettings/
//
// Параметры:
//	СтруктураНастроек - Структура - настройки инстанса, подрробнее о настройках можно посомотреть в документации
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Подробнее https://green-api.com/v3/docs/api/account/SetProfilePicture/
//
// Параметры:
//	ПутьКФайлу - Строка - путьк артинки для установки аватара
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
//	Подробнее https://green-api.com/v3/docs/api/account/GetAccountSettings/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьИнформациюОбАккаунте() Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getAccountSettings"), Истина);
	Возврат Ответ;
...
// Подробнее https://green-api.com/v3/docs/api/sending/SendMessage/
//
// Параметры:
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьТекст(ИдЧата, Сообщение) Экспорт
...
// Сообщение будет добавлено в очередь на отправку. Скорость отправки сообщений из очереди регулирует 
// параметр Интервал отправки сообщений.
// Подробнее https://green-api.com/v3/docs/api/sending/SendFileByUpload
//
// Параметры:
//  Телефон - Число - Номер мобильного телефона
//  Заголовок - Строка - Представление файла
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьВидеоАудиоИзображениеДокумент(ИдЧата, ПутьКФайлу, Заголовок = "") Экспорт
	...
	ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель);
	ЗаписьДанных.Записать(ПараметрFormData("chatId", ИдЧата));
	
	Если ЭтоКартинкаИлиВидео(ПутьКФайлу) Тогда
		//
		ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель);
		ЗаписьДанных.Записать(ПараметрFormData("caption", Заголовок));
	Иначе
		//
...
// Подробнее https://green-api.com/v3/docs/api/sending/SendFileByUrl/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьВидеоАудиоИзображениеДокументПоURL(ИдЧата, УРЛФайла, ИмяФайла, Заголовок = "") Экспорт
...
// Метод предназначен для загрузки файла в облачное хранилище, который можно отправить методом sendFileByUrl.
//	А так же позволяет избежать ошибок получения файлов из внешних хранилищ и ускорить отправку сообщений,
//	например, в сценарии рассылки.
// Подробнее https://green-api.com/v3/docs/api/sending/UploadFile/
//
// Параметры:
...
Функция ЗагрузитьФайл(ПутьКФайлу) Экспорт
	...
	Заголовки = Новый Соответствие();
	// Для корректного определения типа файла требуется
	// передавать тип файла в заголовке "Content-Type"
	// или передвать имя и расширение файла в заголовке "GA-Filename"
	Заголовки.Вставить("Content-Type", ПолучитьMimeTypeПоРасширению(Файл.Расширение));	
	Заголовки.Вставить("GA-Filename" , Файл.Имя);
...
// Подробнее https://green-api.com/v3/docs/api/sending/SendLocation/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьГеолокацию(ИдЧата, Широта, Долгота) Экспорт
...
// Подробнее https://green-api.com/v3/docs/api/sending/SendContact/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
//  ИдКонтакта - Строка - идентификатор аонтакта который отправляем
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОтправитьКонтакт(ИдЧата, ИдКонтакта) Экспорт
	...
	Контакт.Вставить("chatId", ИдКонтакта);
...
// Подробнее https://green-api.com/v3/docs/api/receiving/technology-http-api/ReceiveNotification/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
Функция СкачатьФайлПоСсылке(downloadUrl)
	...
	Если Не ЗначениеЗаполнено(Хост) Или Не ЗначениеЗаполнено(Адрес) Тогда		
		...
		//ВызватьИсключение СтрШаблон("Не удалось получить ссылку на файл", "", "");	
	КонецЕсли;
	...
	Если Ответ.КодСостояния = 200 Тогда
		РасширениеФайла = ПолучитьMimeTypeПоРасширению(Ответ.Заголовки["Content-Type"]);
...
// Подробнее https://green-api.com/v3/docs/api/receiving/files/DownloadFile/
//
// Параметры:
//  ИдЧата - Строка - идентификатор чата
...
// Подробнее https://green-api.com/v3/docs/api/journals/GetChatHistory/
// 
// Параметры:
//  ИдЧата - Строка - идентификатор чата	
//	КоличествоСообщений - число - Время в минутах, за которое требуется показать сообщения (по умолчанию равно 1440 минут)
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/journals/GetMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор чата
//  ИдСообщения - Строка - Идентификатор сообщения
...
// Подробнее https://green-api.com/v3/docs/api/journals/LastIncomingMessages/
//
// Параметры:
//	Интервал - число - Время в минутах, за которое требуется показать сообщения (по умолчанию равно 1440 минут)
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/journals/LastOutgoingMessages/
// 
// Параметры:
//	Интервал - число - Время в минутах, за которое требуется показать сообщения (по умолчанию равно 1440 минут)
//
...
// Метод предназначен для получения количества сообщений в очереди на отправку в мессенджере MAX.
//	Каждое уведомление хранится в очереди 24 часа. Информация в методе GetMessagesCount обновляется 1 раз в 10 секунд.
// Подробнее https://green-api.com/v3/docs/api/queues/GetMessagesCount/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьКоличествоСообщенийКОтправке() Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getMessagesCount"));
	Возврат Ответ;
...
// Подробнее https://green-api.com/v3/docs/api/queues/ShowMessagesQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьОчередьСообщенийКОтправке() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "showMessagesQueue"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для очистки очереди сообщений на отправку.
// Подробнее https://green-api.com/v3/docs/api/queues/ClearMessagesQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОчиститьОчередьСообщенийКОтправке() Экспорт
	...
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "clearMessagesQueue"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для получения количества уведомлений во входящей очереди.
//	Каждое уведомление хранится в очереди 24 часа. Информация в методе GetMessagesCount обновляется 1 раз в 10 секунд.
// Подробнее https://green-api.com/v3/docs/api/queues/GetWebhooksCount/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ПолучитьКоличествоУведомленийКОтправке() Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Ответ = ОтправитьGETЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "getWebhooksCount"));
	Возврат Ответ;
	
КонецФункции

// Метод предназначен для очистки входящей очереди уведомлений.
// Подробнее https://green-api.com/v3/docs/api/queues/ClearWebhooksQueue/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ОчиститьОчередьВходящихУведомлений() Экспорт
	
	Инстанс = IdInstance;
	Токен = ApiToken;
	
	Ответ = ОтправитьDELETEЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "clearWebhooksQueue"));
	Возврат Ответ;
...
// Подробнее https://green-api.com/v3/docs/api/groups/CreateGroup/
//
// Параметры:
//  ИмяГруппы - Строка - Наименование нового группового чата
//  ИдЧатов - Массив - Коллекция ид чатов(строки) участников группы
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция СоздатьГруппу(ИмяГруппы, ИдЧатов) Экспорт
	...
	Структура.Вставить("chatIds", СтрСоединить(ИдЧатов, ","));
...
// Подробнее https://green-api.com/v3/docs/api/groups/UpdateGroupName/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//  ИмяГруппы - Строка - Новое имя группового чата
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ИзменитьИмяГруппы(ИдГруппы, ИмяГруппы) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("chatId", ИдГруппы);
	Структура.Вставить("groupName", ИмяГруппы);
...
// Подробнее https://green-api.com/v3/docs/api/groups/GetGroupData/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//
...
// Подробнее https://green-api.com/v3/docs/api/groups/AddGroupParticipant/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
...
// Подробнее https://green-api.com/v3/docs/api/groups/RemoveGroupParticipant/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//  ИдУчастника - Строка - Идентификатор участника, добавляемого в групповой чат.
...
// Подробнее https://green-api.com/v3/docs/api/groups/SetGroupAdmin/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//  ИдУчастника - Строка - Идентификатор участника, которому добавляем админа.
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/groups/RemoveAdmin/
//
// Параметры:
//  ИдГруппы - Строка - Идентификатор группового чата
//  ИдУчастника - Строка - Идентификатор участника, у которого отбираем админа.
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/groups/LeaveGroup/
//
// Параметры:
...
// Подробнее https://green-api.com/v3/docs/api/groups/SetGroupPicture/
//
// Параметры:
...
Функция УстановитьФотоГруппы(ИдГруппы, ПутьКФайлу) Экспорт
	...
	ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель);
	ЗаписьДанных.Записать(ПараметрFormData("chatId", ИдГруппы));
	ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель);
	ЗаписьДанных.Записать(ФайлFormData("file", ПутьКФайлу, ДвоичныеДанные));
	ЗаписьДанных.ЗаписатьСтроку("");
	ЗаписьДанных.ЗаписатьСтроку("--" + Разделитель + "--");
...
// Подробнее https://green-api.com/v3/docs/api/marks/ReadChat/
//
// Параметры:
...
// Метод проверяет наличие аккаунта MAX на номере телефона.
// Подробнее https://green-api.com/v3/docs/api/service/CheckAccount/
//
// Параметры:
...
//  ЧтениеJSON  - Ответ сервера
//
Функция ПроверитьНаличиеМакс(Телефон) Экспорт
	...
	Возврат ОтправитьPOSTЗапрос(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "checkAccount"), Тело);
	
КонецФункции

// Метод возвращает аватар корреспондента или группового чата.
// Подробнее https://green-api.com/v3/docs/api/service/GetAvatar/
//
// Параметры:
...
// Подробнее https://green-api.com/v3/docs/api/service/GetContacts/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/service/GetContactInfo/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/api/service/DeleteMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
...
// Метод изменяет сообщение из чата.
// Подробнее https://green-api.com/v3/docs/api/service/EditMessage/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
//  ИдСообщения - Строка - Идентификатор изменяемого сообщения
//  Сообщение   - Строка - новый текст сообщения
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
Функция ИзменитьСообщение(ИдЧата, ИдСообщения, Знач Сообщение) Экспорт
	...
	Возврат ОтправитьPOSTЗапросБезОтвета(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "editMessage"), Тело);
	
КонецФункции

// Метод предназначен для получения списка чатов текущего аккаунта.
// Подробнее https://green-api.com/v3/docs/api/service/GetChats/
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
// 
...
// https://green-api.com/v3/docs/api/service/SendTyping/
//
// Параметры:
//  ИдЧата - Строка - Идентификатор корреспондента или группового чата
//	ВремяПоказа - Число - Время показа уведомления набора сообщения в чате собеседника.
//					Время ограничено значениями от 1000 до 20000 миллисекунд (от 1 до 20 секунд).
// 
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
// 
Функция ОтправитьУведомлениеОНабореТекста(ИдЧата, ВремяПоказа = 1000) Экспорт
	...
	Структура = Новый Структура;
	Структура.Вставить("chatId", ИдЧата);
	Структура.Вставить("typingTime", ВремяПоказа);
	
	Тело = СериализоватьВJSONСтроку(Структура);
	
	Возврат ОтправитьPOSTЗапросБезОтвета(ХостПоУмолчанию(), URLМетодаСервиса(Инстанс, Токен, "sendTyping"), Тело);
...
// Подробнее https://green-api.com/v3/docs/partners/getInstances/
//
// Возвращаемое значение:
//  ЧтениеJSON  - Ответ сервера
//
...
// Подробнее https://green-api.com/v3/docs/partners/deleteInstanceAccount/
//
// Параметры:  
...
Функция ОтправитьGETЗапрос(Хост, Адрес, Защищенное = Истина)
...
Функция ОтправитьDELETEЗапрос(Хост, Адрес, Защищенное = Истина)
...
Функция ХостМедиаПоУмолчанию() Экспорт 
	...
	Если ЗначениеЗаполнено(mediaHost) Тогда	
		Результат = mediaHost;
	Иначе 
...
Функция URLМетодаСервиса(Инстанс, Токен, Метод)
	Возврат СтрШаблон("v3/waInstance%1/%2/%3", Инстанс, Метод, Токен);
КонецФункции

Функция URLМетодаСервисаПартнер(ТокенПартнера, Метод)
	Возврат СтрШаблон("partner/%1/%2", Метод, ТокенПартнера);
КонецФункции  
...
Функция ВерсияОбработки() Экспорт
	Возврат "Green API-MAX v" + НомерВерсии();
КонецФункции
